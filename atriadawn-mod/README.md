...
**Без локальной сборки:** нажмите *Run workflow* на вкладке **Actions**
(`Build Atria Dawn Chat mod`) — jar появится в артефактах запуска. При пуше
тега (например `v1.0.0`) jar автоматически приложится к GitHub Release.

> В этом монорепозитории за сборку отвечает корневой воркфлоу
> `.github/workflows/build-atriadawn.yml`. Если решите вынести мод в
> отдельный репозиторий — просто скопируйте **содержимое** этой папки в корень
> нового репозитория: внутри уже лежит собственный
> `atriadawn-mod/.github/workflows/build.yml`, который подхватится автоматически.
