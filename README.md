...
- `/atria reset` — забыть контекст диалога; `/atria about` — справка;
  `/atria key <ключ>` — вписать API-ключ (операторы); `/atria key status` — проверить ключ;
...
### Настройка (один раз)
1. Запустите игру с модом (в одиночной игре — с включёнными читами).
2. Получите API-ключ в консоли Atria (ключи вида `atr_...`) и введите его **прямо в игре**:
   ```
   /atria key atr_ВАШ_КЛЮЧ
   ```
   Ключ сохранится в `config/atriadawn.json`, перезагрузка не нужна — можно сразу писать боту.
   Проверить: `/atria key status` (ключ показывается замаскированным), удалить: `/atria key clear`.

   **Альтернатива** — отредактировать файл `config/atriadawn.json` вручную и выполнить `/atria reload`:
   ```json
   {
...
- ⚠️ Вопросы отправляются на внешний API — не пишите боту пароли и личные данные.
  API-ключ хранится в `config/atriadawn.json` в открытом виде (в том числе введённый
  через `/atria key`) — не давайте файл посторонним.
