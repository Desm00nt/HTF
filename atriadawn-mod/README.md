...
**Без локальной сборки:** перенесите мод в отдельный репозиторий (скопировав
**содержимое** этой папки в корень) — внутри уже лежит собственный workflow
`atriadawn-mod/.github/workflows/build.yml`, и GitHub Actions сам соберёт jar
(артефакт на странице запуска, а при пуше тега `v1.0.0` — сразу в Release).

> Примечание: GitHub не разрешает этой сессии создавать корневые
> `.github/workflows/*`, поэтому в монорепозитории мод не собирается корневым
> CI — только локально или в собственном репозитории (см. выше).
